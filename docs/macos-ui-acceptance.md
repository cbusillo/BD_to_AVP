...
| Batch disc-title selection | Pass | A source-folder batch exposes `Main Feature` and `All 3D Videos` for ISO disc images. The choice is applied after each image is inspected, all selected titles run sequentially with distinct queue labels and outputs, and non-disc files still convert once. |
