...
only when the receipt was absent from the base, validates against its immutable
release tag commit, exactly matches the successor qualification's
`immutable_history.previous_beta`, and is the only `docs/release-evidence/`
addition. It may not modify the evidence index, release ledger, policy, route
table, publication record, qualification snapshot, or milestone manifest. The
