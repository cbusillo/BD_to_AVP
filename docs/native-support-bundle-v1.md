...
- `worker`: Worker version, active state, ephemeral PID/process-group ID, and
...
- `privacy`: Human-readable included/excluded categories, redaction-rules
  version, and bundle-only token scope.
...
choices, result size, worker version, process-group ID, exit status, and a
...
directory/read/write booleans, file size, modification age, free/total capacity,
read-only state, and a normalized error category. Volume names, device paths,
filesystem labels, filenames, and exception descriptions are excluded. Missing
and inaccessible paths are represented explicitly.
...
- Full paths, file URLs, home-relative paths, Windows paths, volume paths, and
  known filenames become tokens such as `<path:89ABCDEF:001>`.
...
- Authorization values, bearer tokens, API keys, passwords, secrets,
  GitHub/AWS-style credentials, JWTs, serial/device identifiers, long
  hexadecimal identifiers, email addresses, IP addresses, and MAC addresses
  are removed.
...
commands, source titles, raw paths, serials, or reusable file fingerprints.
