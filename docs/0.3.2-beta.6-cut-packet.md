...
> was deleted under separate explicit authorization on August 21, 2026; the
> immutable deletion disposition records verified absence of both draft and tag.
...
unpublished draft `374538590`, which was later deleted through the authorized
recovery path; no Git tag, public release, Pages appcast item, PyPI version, or
Homebrew change was created.
...
- At cancellation and immutable recording, draft `374538590` retained the exact
  DMG, checksum, appcast, and release receipt. Their IDs, sizes, and SHA-256
  digests remain recorded in
...
- `.github/release-freezes.json` blocks reuse of `v0.3.2-beta.6`. Build `168`
  ...
  `immutable_history.burned_builds`. The authorized deletion disposition now
  satisfies the cancelled-attempt recovery gate without making build `168`
  reusable.

The draft was diagnostic evidence, not a resumable release candidate after the
fixture-hardening changes merged. It was deleted at `2026-08-21T22:58:29Z` by
`cbusillo` under explicit authorization. The reviewed disposition at
`docs/release-attempts/v0.3.2-beta.6/draft-deletion-v1.json` binds the immutable
attempt digest, receipt digest, draft ID, source SHA, authorization actor and
canonical fingerprint, deletion time, and verified absence of both draft and
tag. Do not mutate the cancelled-attempt record, checked receipt, or deletion
disposition.
