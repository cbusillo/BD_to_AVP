...
> **Published and immutable.** Beta 6 was published on July 22, 2026 through
...
[release-routes.md](release-routes.md). Beta 6 was released from protected
`main` commit `7bf7858057e988a6eff753d2588bebe60dfd660e` after the diagnostic
inventory, Worker stdout backpressure fix, and native MVC decoder recovery
landed with exact-main CI and CodeQL green.
...
Build `151` is globally newer than immutable Beta 5 build `150`. The release
was published at `2026-07-22T21:37:05Z` with the expected DMG,
...
Beta 6 was appended above the immutable production history:
...
Stable and RC exclude every Beta item. Beta and Alpha admit Beta 3 through
Beta 6. Moving to a safer route never downgrades an installed prerelease.
...
The explicit Beta 6 request authorized metadata preparation, protected-main
review, and exact-SHA dispatch. A separate run-bound `macos-signing` approval
authorized credential use in the publication flow and was bound to the
published run and protected-main commit. Later release packets may refer to
Beta 6 only as immutable production history.
