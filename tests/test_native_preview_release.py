...
class NativePreviewWorkflowTests(unittest.TestCase):
    def test_workflow_is_manual_main_only_and_uses_release_runner(self) -> None:
        ...
        self.assertIn("moved after preview signing approval", str(package))
        self.assertGreaterEqual(str(package).count("refs/remotes/origin/main"), 4)
        self.assertIn("moved immediately before preview certificate use", str(package))
        self.assertIn("moved immediately before preview notarization credential use", str(package))
        self.assertIn("moved immediately before preview package signing", str(package))
